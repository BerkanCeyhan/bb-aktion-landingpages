...
export function DesireSection() {
    ...
    return (
        <section ref={sectionRef} className="py-24 bg-dark-surface2 relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-brand-volt/5 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
                ...
                <div className="grid md:grid-cols-3 gap-6">
                    {testimonials.map((testi, idx) => (
                        <div
                            key={idx}
                            ref={addToRefs}
                            className="bg-dark-bg p-8 border-l-2 border-brand-volt relative hover:bg-dark-surface transition-colors duration-300"
                        >
                            <QuoteIcon className="absolute top-6 right-6 text-dark-surface2 w-12 h-12" />
                            <div className="flex items-center gap-1 mb-4 relative z-10">
                                {[...Array(testi.rating)].map((_, i) => (
                                    <Star key={i} className="text-brand-orange fill-brand-orange" size={16} />
                                ))}
                            </div>
                            <p className="text-dark-text font-sans mb-8 relative z-10 italic">"{testi.text}"</p>
                            <div className="flex items-center gap-4 border-t border-dark-surface2 pt-4 relative z-10">
                                <div className="w-10 h-10 bg-dark-surface2 rounded-full flex items-center justify-center font-drama text-brand-volt">
                                    {testi.initial}
                                </div>
                                <div>
