            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="space-y-4">
                    {faqs.map((faq, idx) => (
                        <div
                            key={idx}
                            className={`rounded-2xl border transition-colors duration-300 ${openIndex === idx ? 'border-brand-orange bg-brand-orange/5' : 'border-dark-surface2 bg-dark-surface hover:border-brand-orange/40'}`}
                        >
                            <button
                                onClick={() => setOpenIndex(openIndex === idx ? -1 : idx)}
                                className="w-full text-left px-6 py-5 flex items-center justify-between focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-orange rounded-2xl"
                            >
                                <span className="font-drama text-lg md:text-xl pr-8 text-dark-text">
                                    {faq.q}
                                </span>
                                <ChevronDown
                                    className={`text-brand-orange transition-transform duration-300 transform shrink-0 ${openIndex === idx ? 'rotate-180' : ''}`}
                                    size={24}
                                />
                            </button>
                            ...
                            >
                                <div className="px-6 pb-6 pt-2 border-t border-brand-orange/20 mt-2">
...
export function GuaranteeFooter() {
    ...
    return (
        <footer className="bg-dark-surface py-20 border-t-4 border-brand-orange relative overflow-hidden">
            {/* Warm wash behind shield */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] h-[340px] bg-brand-yellow/25 rounded-full blur-[90px] pointer-events-none" />

            <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
                <ShieldCheck className="w-24 h-24 mx-auto text-brand-teal mb-8" />
