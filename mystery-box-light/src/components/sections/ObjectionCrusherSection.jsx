...
export function ObjectionCrusherSection() {
    ...
    return (
        <section className="py-24 bg-brand-bg border-t border-brand-surface2 relative">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">

                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-5xl font-heading text-brand-orange tracking-tight">EHRLICH GEFRAGT. <br /><span className="text-brand-orange">EHRLICH GEANTWORTET.</span></h2>
                </div>
                ...
                <div className="space-y-4">
                    {faqs.map((faq, idx) => (
                        <div
                            key={idx}
                            className={`border transition-colors duration-300 ${openIndex === idx ? 'border-brand-volt bg-brand-surface' : 'border-brand-surface2 bg-brand-bg hover:border-brand-muted'}`}
                        >
                            <button
                                onClick={() => setOpenIndex(openIndex === idx ? -1 : idx)}
                                className="w-full text-left px-6 py-5 flex items-center justify-between focus:outline-none"
                            >
                                <span className={`font-drama text-lg md:text-xl pr-8 ${openIndex === idx ? 'text-brand-volt' : 'text-brand-text'}`}>
                                    {faq.q}
                                </span>
                                <ChevronDown
                            ...
                            >
                                <div className="px-6 pb-6 pt-2 border-t border-brand-surface2 mt-2">
                                    <p className="text-brand-muted font-sans text-lg leading-relaxed">{faq.a}</p>
                                </div>
                            </div>
                        </div>
                    ))}
...
export function GuaranteeFooter() {
    ...
    return (
        <footer className="bg-brand-surface py-20 border-t-4 border-brand-orange relative overflow-hidden">
            {/* Glow behind shield removed */}

            <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
                <ShieldCheck className="w-24 h-24 mx-auto text-brand-orange mb-8" />

                <h2 className="text-3xl md:text-5xl font-drama text-brand-orange uppercase tracking-tighter mb-6">
                    MEHR WERT ALS DEIN KAUFPREIS.<br /> GARANTIERT.
                </h2>

                <p className="text-xl text-brand-muted font-sans mb-12 max-w-2xl mx-auto">
                ...
                <div className="mt-16 pt-8 border-t border-brand-surface2 text-sm font-mono text-brand-muted flex flex-col items-center gap-4">
