...
export function InterestSection() {
    ...
    return (
        <section ref={sectionRef} className="py-24 bg-brand-bg border-y border-brand-surface2 relative">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

                <div className="text-center max-w-3xl mx-auto mb-16">
                    <h2 className="text-3xl md:text-5xl font-heading text-brand-orange mb-6">
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {features.map((feature, idx) => (
                        <div
                            key={idx}
                            ref={addToCards}
                            className="bg-brand-surface p-8 border border-brand-surface2 hover:border-brand-volt/50 transition-colors duration-300 group"
                        >
                            <div className="transform group-hover:scale-110 transition-transform duration-300 origin-left">
                                {feature.icon}
                            </div>
                            <h3 className="text-xl font-drama text-brand-orange mb-3 tracking-wide">{feature.title}</h3>
                            <p className="text-brand-muted font-sans leading-relaxed">{feature.desc}</p>
                        </div>
                    ))}
                </div>
