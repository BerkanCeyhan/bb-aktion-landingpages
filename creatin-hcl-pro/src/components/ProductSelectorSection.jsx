...
export default function ProductSelectorSection() {
    ...
    return (
        <section id="checkout-section" className="py-24 px-4 bg-brand-sand relative border-t border-brand-text/15">
            ...
            <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 lg:gap-16 items-center bg-brand-surface border border-brand-text/10 edge-emboss p-4 md:p-12 relative overflow-hidden z-10">

                {/* Spec corner */}
                <span className="absolute top-4 right-5 font-mono text-[10px] uppercase tracking-wider text-brand-text/30 hidden md:block">03 — Dein Vorrat</span>
                ...
                <div className="w-full lg:w-5/12 relative z-10 flex flex-col justify-center items-center text-center lg:text-left lg:items-start">
                    <div className="w-full max-w-md mb-8 relative">
                        ...
                        <img
                            ...
                            className="w-full h-auto object-contain aspect-[4/5] md:aspect-square border border-brand-text/10 rounded-xl relative z-10 shadow-2xl transition-all duration-500"
                        />
                    </div>
                </div>

                <div className="w-full lg:w-7/12 relative z-10">
                    <h2 className="text-3xl font-heading uppercase mb-2">Creatin HCL 1000mg</h2>
                    <div className="flex gap-4 mb-4 text-brand-text/50 text-xs font-mono uppercase border-b border-brand-text/10 pb-4">
                    ...
                    <p className="text-brand-text/60 mb-8 font-medium">Wähle deinen Vorrat. Bündel-Käufe sichern dir den besten Preis pro Tagesdosis.</p>
                    ...
                    <div className="flex flex-col gap-4 mb-8">
                        {BUNDLES.map((bundle) => {
                            ...
                            return (
                                ...
                                >
                                    {bundle.popular && (
                                        <div className="absolute -top-3 right-4 bg-brand-accent text-brand-surface text-[10px] font-drama font-bold uppercase px-2 py-1 tracking-wider shadow-lg">
                                            Beliebteste Wahl
                                        </div>
                                    )}
                                    <div>
                                        <h3 className={`font-heading uppercase text-xl ${isSelected ? 'text-brand-accent' : 'text-brand-text'}`}>
                                            {bundle.name}
                                        </h3>
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mt-1">
                                            <p className="text-xs text-brand-text/50 font-mono">{bundle.subtitle}</p>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="text-[10px] text-brand-accent font-mono mt-1">
                    ...
                    <div className="flex items-center justify-center gap-6 mt-6 text-[10px] font-mono uppercase text-brand-text/40">
                        <span className="flex items-center gap-1"><Box className="w-3 h-3 text-brand-text/30" /> Kostenloser Versand ab 60€</span>
                        <span className="flex items-center gap-1"><Shield className="w-3 h-3 text-brand-text/30" /> Sichere Bezahlung</span>
                    </div>
                </div>
