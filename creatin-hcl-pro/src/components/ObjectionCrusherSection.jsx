...
export default function ObjectionCrusherSection() {
    ...
    return (
        <section className="py-24 px-4 bg-brand-surface border-y border-brand-text/15 relative overflow-hidden">
            <div className="absolute inset-0 tex-grain pointer-events-none" />

            {/* Background Graphic */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[20vw] font-heading text-brand-text/[0.035] leading-none whitespace-nowrap pointer-events-none select-none">
                NO BULLSHIT
            </div>

            <div className="max-w-4xl mx-auto relative z-10">
                <span className="block text-center font-mono text-brand-accent uppercase tracking-[0.25em] text-xs mb-5">
                    04 — Klartext
                </span>
                <h2 className="text-3xl md:text-6xl font-heading uppercase mb-12 text-center leading-[0.9]">
                ...
                <div className="flex flex-col gap-4">
                    {OBJECTIONS.map((obj, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`border transition-all duration-300 ${isOpen ? 'border-brand-accent/50 bg-brand-bg' : 'border-brand-text/10 bg-brand-bg/50'}`}
                            >
                                <button
                                    onClick={() => setOpenIndex(isOpen ? -1 : idx)}
                                    className="w-full text-left p-6 md:p-8 flex items-center justify-between gap-4 font-heading uppercase text-lg md:text-xl tracking-wide text-brand-text hover:text-brand-accent transition-colors"
                                >
                                    <span>{obj.question}</span>
                                    <div className={`p-2 border rounded-none transition-colors ${isOpen ? 'bg-brand-accent text-brand-bg border-brand-accent' : 'border-brand-text/20 text-brand-text/50'}`}>
